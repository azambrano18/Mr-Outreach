...
describe('SequenceExecutionsService', () => {
  ...
  let templateVersions: jest.Mocked<Pick<SequenceTemplateVersionRepository, 'findLatestByTemplate' | 'findById'>>;
  ...
  beforeEach(() => {
    ...
    templateVersions = { findLatestByTemplate: jest.fn().mockResolvedValue(acceptedVersion), findById: jest.fn() };
  ...
  it('rejects when the latest version is not ACCEPTED', async () => {
    templateVersions.findLatestByTemplate.mockResolvedValue({ ...acceptedVersion, status: 'FAILED' } as any);
    await expect(service.create(orgId, executiveId, { mailboxId, templateId })).rejects.toBeInstanceOf(ConflictException);
  });
