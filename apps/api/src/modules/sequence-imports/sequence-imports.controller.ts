...
export class SequenceImportsController {
  ...
  /**
   * Fase 2, Caso B — single, atomic, idempotent confirmation. Replaces the
   * old `SequenceImportsService.confirm()` + deferred
   * `SUBMITTED → advance → materialize` path — that path is no longer the
   * functional way to confirm an import (see `advance()`'s own guard
   * below), kept only as inert legacy surface for already-submitted rows
