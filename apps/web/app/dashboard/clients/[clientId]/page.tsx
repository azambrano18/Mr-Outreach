...
export default async function ClientDetailPage({ params }: { params: { clientId: string } }) {
  ...
  const domainsBase = canReadAll ? '/clients' : '/me/clients';

  let client: ManagedClientSummary;
  let domains: DomainSummary[] = [];
  try {
    [client, domains] = await Promise.all([
      apiFetch<ManagedClientSummary>(`${clientBase}/${params.clientId}`),
      apiFetch<DomainSummary[]>(`${domainsBase}/${params.clientId}/domains`),
    ]);
  } catch (error) {
  ...
  if (!canReadAll) {
    // Executives keep the existing simpler, read-only view — they never
    // manage domains/mailboxes/assignments, only browse them.
    return (
          ...
          {domains.length === 0 ? (
            ...
            <ul className="flex flex-col gap-3">
              {domains.map((domain) => (
                <li key={domain.id}>
                  <Link
                    href={`/dashboard/clients/${client.id}/domains/${domain.id}`}
  ...
  const canManageDomains =
    currentUser.permissions.includes('domains.create') && currentUser.permissions.includes('domains.update');
  const canAssign = currentUser.permissions.includes('clients.assign');
  const canReadAudit = currentUser.permissions.includes('audit.read');
  const canReadMailboxes = currentUser.permissions.includes('mailboxes.read.all');

  const [mailboxes, assignees, activeExecutives, auditLog] = await Promise.all([
    canReadMailboxes
      ? apiFetch<MailboxSummary[]>(`/clients/${params.clientId}/mailboxes`).catch(() => [])
      : Promise.resolve<MailboxSummary[]>([]),
    apiFetch<ClientAssigneeSummary[]>(`/clients/${params.clientId}/assignees`).catch(() => []),
    canAssign
      ? apiFetch<UserSummary[]>('/users').then((users) => users.filter((u) => u.status === 'ACTIVE'))
      : Promise.resolve<UserSummary[]>([]),
    canReadAudit
      ? apiFetch<AuditLogEntry[]>(`/clients/${params.clientId}/audit-log`).catch(() => [])
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-6">
      ...
      >
        Todas las conversaciones del cliente
        {client.pendingConversationCount > 0 && (
          <span className="rounded-full bg-white/20 px-1.5 py-0.5 text-xs">{client.pendingConversationCount}</span>
        )}
      </Link>
